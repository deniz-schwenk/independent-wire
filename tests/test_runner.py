...
def test_build_production_stages_returns_three_lists():
    ...
    assert len(topic_stages) == 25, f"expected 25 production topic-stages (V2-06b + prune + cleanup_stale_references + filter_media_actors_quoted + consolidate_actors + propagate_outlet_metadata + resolve_actor_aliases + partition_canonical_actors_by_evidence + consolidate_missing_coverage + derive_single_voices), got {len(topic_stages)}"
...
def test_build_hydrated_stages_returns_three_lists():
    ...
    assert run_stages, "run_stages should not be empty"
    assert len(topic_stages) == 32, f"expected 32 hydrated topic-stages (V2-06b + prune + cleanup_stale_references + filter_media_actors_quoted + consolidate_actors + propagate_outlet_metadata + resolve_actor_aliases + partition_canonical_actors_by_evidence + consolidate_missing_coverage + derive_single_voices), got {len(topic_stages)}"
