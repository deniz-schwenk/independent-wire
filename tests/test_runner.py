...
def test_build_production_stages_returns_three_lists():
    ...
    assert len(topic_stages) == 18, f"expected 18 production topic-stages (V2-06b + prune + filter_media_actors_quoted), got {len(topic_stages)}"
...
def test_build_hydrated_stages_returns_three_lists():
    ...
    assert run_stages, "run_stages should not be empty"
    assert len(topic_stages) == 25, f"expected 25 hydrated topic-stages (V2-06b + prune + filter_media_actors_quoted), got {len(topic_stages)}"
