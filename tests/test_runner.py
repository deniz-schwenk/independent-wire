...
def test_build_production_stages_returns_three_lists():
    ...
    assert len(topic_stages) == 23, f"expected 23 production topic-stages (V2-06b + prune + cleanup_stale_references + filter_media_actors_quoted + consolidate_actors + propagate_outlet_metadata + resolve_actor_aliases + partition_canonical_actors_by_evidence), got {len(topic_stages)}"
...
def test_build_hydrated_stages_returns_three_lists():
    ...
    assert run_stages, "run_stages should not be empty"
    assert len(topic_stages) == 30, f"expected 30 hydrated topic-stages (V2-06b + prune + cleanup_stale_references + filter_media_actors_quoted + consolidate_actors + propagate_outlet_metadata + resolve_actor_aliases + partition_canonical_actors_by_evidence), got {len(topic_stages)}"
