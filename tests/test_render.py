...
def test_select_by_visibility_topicbus_tp():
    ...
    expected = {
        ...
        # Mentioned-actors bracket written by `derive_mentioned_actors`
...
def test_render_tp_public_perspectives_reshape_to_dict():
    ...
    # `mentioned_actors` joined the perspectives sub-dict (renamed from
    # `single_voices` on 2026-05-21; all non-cluster actors now qualify).
    assert set(persp) == {"position_clusters", "missing_positions", "mentioned_actors"}
    ...
    # When the stage has not run, mentioned_actors is an empty dict (the
    # slot's typed default) — rendered through as {} for consumers.
    assert persp["mentioned_actors"] == dict(tb.mentioned_actors or {})
