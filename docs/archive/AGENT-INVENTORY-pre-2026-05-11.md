...
> Superseded by docs/AGENT-IO-MAP.md on 2026-05-11.
...
| `agents/researcher/PLAN-SYSTEM.md` + `PLAN-INSTRUCTIONS.md` | `researcher_plan` | `ResearcherPlanStage` (topic-stage) | `anthropic/claude-opus-4.6` | 0.5 | `none` | 32000 | `tools=[]`. Emits multilingual search query plan. Schema: `RESEARCHER_PLAN_SCHEMA`. Story-Shape-Section in PLAN-INSTRUCTIONS.md (commit b2bec02 + 0b03760): six shapes (Quantitative-claim, Stakeholder-conflict, Policy/regulatory, Crisis/emergency, Tech/business, Cultural/social) as heuristic targeting per query. Date context (run_date) passed in via context block since commit bd92e44. |
...
| `agents/resolve_actor_aliases/SYSTEM.md` + `INSTRUCTIONS.md` | `resolve_actor_aliases` | `ResolveActorAliasesStage` (topic-stage) | `google/gemini-3-flash-preview` | 1.0 | `medium` | 66000 | `tools=[]`. Cross-variant alias resolver — identifies multilingual / paraphrased name variants of the same real-world entity and flags generic source-class labels. Reads `final_actors[]` (post `consolidate_actors`); writes `canonical_actors[]` (alias-merged) plus `actor_alias_mapping[]` (audit trail). Production config = Y, verified by 30-run diagnostic-V2 smoke (5/5 stable across all critical patterns including borderline anonymous-flag cases). The reasoning-enabled profile contradicts the prior fleet-level `reasoning=none` default; see ARCH-V2-BUS-SCHEMA §7.2 Engineering note. Schema: `RESOLVE_ACTOR_ALIASES_SCHEMA`. |
| `agents/perspective/SYSTEM.md` + `INSTRUCTIONS.md` | `perspective` | `PerspectiveStage` (topic-stage) | `anthropic/claude-opus-4.6` | 0.1 | `none` | 32000 | `tools=[]`. Emits position clusters with per-cluster `actor_ids[]` (references to `canonical_actors`); `pc-NNN` IDs, validated `actor_ids`, and per-cluster counts (`n_actors`, `n_sources`, `n_regions`, `n_languages`) attached by `enrich_perspective_clusters` deterministic topic-stage. Reads `final_sources` (in `src-NNN` form) and `canonical_actors` (in `actor-NNN` form, alias-resolved by `ResolveActorAliasesStage`). Schema: `PERSPECTIVE_SCHEMA`. |
| `agents/writer/SYSTEM.md` + `INSTRUCTIONS.md` | `writer` | `WriterStage` (topic-stage) | `anthropic/claude-opus-4.6` | 0.3 | `none` | 32000 | `tools=[]` (web_search disabled in V2-09c2 to close `[web-N]` Bug-1). Reads `final_sources`, `canonical_actors[]` (consumer-facing canonical actor list, surfaced as `actors` in agent context), and `actor_alias_mapping[]` (as `actor_aliases`); emits `[src-NNN]` citations and dereferences `cluster.actor_ids[]` against canonical names so the same real-world actor is cited under one canonical name throughout the article (F2 dedup reaches the published text). Schema: `WRITER_SCHEMA` (`sources[].src_id` post-V2-09e). |
| `agents/qa_analyze/SYSTEM.md` + `INSTRUCTIONS.md` | `qa_analyze` | `QaAnalyzeStage` (topic-stage) | `anthropic/claude-sonnet-4.6` | 0.1 | `none` | 64000 | `tools=[]`. Never use `r-medium` (crashes 2/4 in eval). Reads `final_sources` and emits `[src-NNN]` citations symmetric with Writer. max_tokens 64000 since commit 813c4e4 (problems_found explanations were truncating in long topics). qa_corrections schema (post commit e2e7efd): each item is {proposed_correction: str, correction_needed: bool}; slot-level mirror semantics: emits full `article` only when any(correction_needed), otherwise omits. Schema: `QA_ANALYZE_SCHEMA`. |
| `agents/bias_detector/SYSTEM.md` + `INSTRUCTIONS.md` | `bias_language` | `BiasLanguageStage` (topic-stage) | `anthropic/claude-opus-4.6` | 0.1 | `none` | 32000 | Filename is `bias_detector/`; the registered agent name is `bias_language`. `tools=[]`. Reads `qa_corrected_article`. severity field removed end-to-end (commit 2a41570). Schema: `BIAS_DETECTOR_SCHEMA`. |
...
| File pair | Agent name | Wrapper class | Model | Temp | Reasoning | max_tokens | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `agents/researcher_hydrated/PLAN-SYSTEM.md` + `PLAN-INSTRUCTIONS.md` | `researcher_hydrated_plan` | `ResearcherHydratedPlanStage` (topic-stage) | `anthropic/claude-opus-4.6` | 0.5 | `none` | 16384 | `tools=[]`. Gap-aware plan reads `hydration_pre_dossier` coverage summary (computed by deterministic helper `_build_coverage_summary` internalised into `src/agent_stages.py` in V2-11a). Schema: `RESEARCHER_PLAN_SCHEMA` (same shape as production planner). Same Story-Shape-Section as production. Plan reads coverage_summary computed by _build_coverage_summary helper. |
