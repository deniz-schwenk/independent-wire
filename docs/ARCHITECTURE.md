...
- **Ollama subscription** (`OLLAMA_API_KEY`, flat-rate, $0 marginal) — two production consumers:
  - **`web_search` default** since **2026-07-06** (`IW_SEARCH_PROVIDER=ollama`, `src/tools/web_search.py`), replacing Perplexity/Sonar as a cost-driven interim bridge ahead of the registry endgame. Basis: `scratch/registry-shadow/BACKTEST-3ARM-REPORT.md` (QUALIFIED GO). One-line revert: `IW_SEARCH_PROVIDER=perplexity` (the Sonar path is retained). The parked `feat/registry-a2` registry backend is the eventual $0 replacement. If `OLLAMA_API_KEY` is absent the search path degrades to DuckDuckGo, logged loudly as `provider_used=duckduckgo` — never silent.
    - **Opacity disclosure + transition state (gate review 2026-07-13, D4).** The web-search arm is a **documented opaque / commercial editorial dependency**: result ranking is decided by a commercial actor that IW can neither audit nor influence (news agencies — AFP, Reuters, dpa — hold commercial deals with search providers), structurally the same opaque filtering the project exists to make visible. This is provider-independent (currently Ollama Cloud). It belongs to the same *disclosed-opacity* category as the pipeline's LLM editorial judgments (e.g. the Opus judge in the `bias_language` composite): opaque by nature, but disclosed and logged loudly — never silent. **`merge` mode is a transition, not the destination.** The `merge` provider (`IW_SEARCH_PROVIDER=merge`) runs the registry arm and the web arm concurrently and merges/dedups them deterministically in Python (no LLM in the merge; registry wins on duplicate URLs); replacement of the web arm by the curated registry — the `on_demand` entries of `config/sources.json`, which are versioned, open and community-correctable: *visible* bias instead of invisible bias — remains the end goal. The retirement bar is fixed pre-measurement in **`BACKLOG-WEBSEARCH-SUNSET.md`** (the D2 sunset criterion — a TP-level A/B showing no material loss on perspective breadth, region coverage, and independently-verified-claim count).
