...
| `agents/perspective/SYSTEM.md` + `INSTRUCTIONS.md` | `perspective` | `PerspectiveStage` (topic-stage) | `anthropic/claude-opus-4.6` | 0.1 | `none` | 32000 | `tools=[]`. Emits position clusters with per-cluster `actor_ids[]` (references to `final_actors`); `pc-NNN` IDs, validated `actor_ids`, and per-cluster counts (`n_actors`, `n_sources`, `n_regions`, `n_languages`) attached by `enrich_perspective_clusters` deterministic topic-stage. Reads `final_sources` (in `src-NNN` form) and `final_actors` (in `actor-NNN` form, populated by `consolidate_actors`). Schema: `PERSPECTIVE_SCHEMA`. |
