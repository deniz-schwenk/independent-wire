...
- `role` — the actor's role or title, in English only, regardless of the article's source language.
...
- `position` — one sentence, in English only, describing what the actor says, holds, or does in this article, regardless of the article's source language.
