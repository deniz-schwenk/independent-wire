...
1. Parse the input. Identify the topic assignment (id, title, selection_reason) and the search_results array. Each entry in search_results contains a query string, its language code, and the raw results text from the search provider.
...
3. For each search query, count how many usable sources it yielded. Record the query, its language code, and this count in the research_queries array.

4. From all usable sources across all searches, select up to 15 for the sources array. If more than 15 exist, keep the 15 that maximize diversity by language, region, and perspective — prioritize breadth over depth. If the same article URL appears in multiple search results, include it only once.
...
5. For each source, write a 1-2 sentence summary of what it uniquely adds to the topic based on the search snippet. Be honest about the limits of snippet-based extraction — if the snippet only reveals a headline and a fragment, say what is visible rather than inventing details.
...
6. Compare findings across languages and regions. Identify preliminary divergences — places where sources from different languages or regions frame the story differently, emphasize different aspects, report different facts, or quote different actors. Write each divergence as a single clear statement. Focus on cross-linguistic and cross-regional differences, not differences between individual articles in the same language.

7. Compile languages_searched from all query language codes. Identify any target languages where searches returned no usable journalistic sources and list them in languages_not_available.

8. Assess coverage gaps. Note missing regions, missing stakeholder perspectives, or entire dimensions of the story that no source addresses. Be specific — "No sources from affected country despite being the subject of the regulation" is useful; "could use more sources" is not.

9. Assemble the final JSON object and return it as your complete response. Output nothing before or after it.
...
- "topic_id": String copied from the assignment (e.g., "tp-2026-04-07-001").

- "research_queries": Array of objects, one per search executed. Each has: "query" (exact search string), "language" (ISO code), "results_found" (integer — usable sources from this query).
...
- "languages_searched": Array of ISO language codes for all queries executed.

- "languages_not_available": Array of ISO language codes for target languages that returned no usable sources.
...
RULE 9 — ALWAYS REPORT GAPS. If a language returned no results, it MUST appear in languages_not_available. Missing perspectives MUST appear in coverage_gaps. Silence about gaps is itself a gap.
